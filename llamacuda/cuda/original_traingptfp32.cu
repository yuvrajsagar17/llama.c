...
float *malloc_and_point_parameters(ParameterTensors *params, size_t *param_sizes, int on_device)
{
    ...
    float **ptrs[] = {
        ...
        &params->attprojw, &params->attprojb, &params->ln2w, &params->ln2b, &params->fcw, &params->fcb, &params->fcw_g, &params->fcb_g, // Added params for SwiGLU
...
void fill_in_activation_sizes(size_t *act_sizes, int B, int T, GPT2Config config)
{
    ...
    act_sizes[12] = L * B * T * 4 * C;                   // fch_glu
    act_sizes[13] = L * B * T * 4 * C;                   // fch_swiglu
    act_sizes[14] = L * B * T * C;                       // fcproj
    act_sizes[15] = L * B * T * C;                       // residual3
    act_sizes[16] = B * T * C;                           // lnf
    act_sizes[17] = B * T;                               // lnf_mean
    act_sizes[18] = B * T;                               // lnf_rstd
    act_sizes[19] = B * T;                               // losses
    act_sizes[20] = L * B * T * 3 * C;                   // qkvr
    act_sizes[21] = B * T * max(3 * C, max(NH * T, Vp)); // output / scratch
}
...
float *malloc_and_point_activations(ActivationTensors *acts, const size_t *act_sizes)
{
    float **ptrs[] = {
        ...
        &acts->ln2_rstd, &acts->fch, &acts->fch_glu, &acts->fch_swiglu, &acts->fcproj, &acts->residual3, &acts->lnf,
