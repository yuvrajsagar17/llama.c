...
__global__ void repeat_interleave_dim0_kernel_backward(float *d_input, const float *d_output, int B, int T, int C, int num_repeats)
{
    int b = blockIdx.x * blockDim.x + threadIdx.x;
    int t = blockIdx.y;
    int c = threadIdx.y;
...
{
    dim3 blockDim(256, dim); // 256 threads for rows and `dim` threads for columns
    dim3 gridDim((B + blockDim.x - 1) / blockDim.x, T);

    repeat_interleave_dim0_kernel<<<gridDim, blockDim>>>(output, input, B, T, C, num_repeats);
    cudaDeviceSynchronize();
}
...
void repeat_interleave_dim0_backward(float *d_input, const float *d_output, int B, int T, int C, int num_repeats)
{
    ...
    repeat_interleave_dim0_kernel_backward<<<gridDim, blockDim>>>(d_input, d_output, B, T, C, num_repeats);
    cudaDeviceSynchronize();
}
...
__device__ void filter_input(float *filtered_inp, int *indices, float *inp, int *flat_expert_indices, int B, int T, int C, int expert_idx, int *count)
{
    int idx = 0;
    for (int b = 0; b < B; ++b)
    {
...
__device__ void store_filtered_output(float *out, float *filtered_out, int *indices, int count, int C)
{
...
void forward_with_experts(float *out, float *w1, float *b1, float *w2, float *b2, float *w3, float *b3, float *inp, int *flat_expert_indices, float *swiglu_xw, float *swiglu_xv, float *int B, int T, int C, int num_experts)
{
    for (int idx = 0; idx < num_experts; ++idx) // NUM_EXPERTS = 4
    {
        ...
        filter_input(filtered_input_xw, indices, input_xw, flat_expert_indices, B, T, 4 * C, idx, &count);
        filter_input(filtered_gate_xv, indices, gate_xv, flat_expert_indices, B, T, 4 * C, idx, &count);
        ...
        store_filtered_output(input_xw, filtered_input_xw, indices, count, 4 * C);
        store_filtered_output(gate_xv, filtered_gate_xv, indices, count, 4 * C);
...
void moe_expert_backward(float *d_filtered_inp, float *d_w1, float *d_b1, float *d_w2, float *d_b2, float *d_w3, float *d_b3,
...
typedef struct
{
    ...
    int *moe_topk_ind; // (L, B, T, num_experts_per_token)
