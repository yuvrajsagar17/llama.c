...
__global__ void topk_kernel(const float *scores, float *top_values, int *top_indices, int rows, int cols, int k)
{
...
void topk(const float *d_scores, float *d_top_values, int *d_top_indices, int batch_size, int seq_len, int num_experts, int k)
{
    ...
    topk_kernel<<<grid_size, block_size, shared_memory_size>>>(d_scores, d_top_values, d_top_indices, rows, cols, k);
...
int main()
{
    ...
    // Call top-k function
    topk(d_scores, d_top_values, d_top_indices, batch_size, seq_len, num_experts, k);
